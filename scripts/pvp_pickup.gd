extends Area3D
## Bonus PvP synchronise : l'hote decide qui le ramasse.
...
func _process(delta: float) -> void:
	...
	position.y = base_y + sin(life * 3.2) * 0.12
	if life > LIFETIME:
		if multiplayer.is_server() and pvp_arena != null:
			pvp_arena.server_pvp_pickup_expired(pickup_id)
		queue_free()
	elif life > LIFETIME - 2.0:
		visible = fmod(life, 0.22) > 0.08

func _on_body_entered(body: Node3D) -> void:
	if not multiplayer.is_server() or pvp_arena == null:
		return
	if body.is_in_group("pvp_players") and not body.dead:
		pvp_arena.server_pvp_pickup_collected(pickup_id, body.pvp_peer_id, type)
		queue_free()
